...
def GetWallCut(fi,wall,_face):
	...
	try:
		...
		BoundingBox.ToCuboid(fi.get_BoundingBox(doc.ActiveView).ToProtoType())
		#geom = fi.get_BoundingBox(doc.ActiveView).ToProtoType()
		return geom
...
#------------------------------------------------------Включаем расчет по высоте стен--------------------------------------------------
			if crv != None and IN[4]:
				#crv = clr.Convert(crv, Autodesk.Revit.DB.Curve).CreateTransformed(transform_Z)
